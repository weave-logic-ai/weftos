...
fn god_node_article(
    ...
    let entity_comm = build_entity_community_map(kg);
    let community_name = entity_comm
        .get(&nid.to_hex())
...
pub fn to_wiki(
    ...
    std::fs::create_dir_all(output_dir)
        .map_err(|e| GraphifyError::ExportError(format!("failed to create wiki dir: {e}")))?;
    ...
    for (cid, nodes) in &communities {
        ...
        std::fs::write(output_dir.join(&filename), &article)
            .map_err(|e| GraphifyError::ExportError(format!("failed to write {filename}: {e}")))?;
        count += 1;
    }
    ...
    std::fs::write(output_dir.join("index.md"), &idx)
        .map_err(|e| GraphifyError::ExportError(format!("failed to write index.md: {e}")))?;
